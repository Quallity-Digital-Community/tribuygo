...
const getBusId = async obj =>{return await Trip.findOne({where :{id:obj.tripId}});}
...
const updateLocation = async ({tripId, longitude, latitude}) =>{
  //console.log("Trip location updated: ", longitude, " : ", latitude);
  return await Trip.update({longitude : longitude, latitude :latitude}, {where:{id :tripId}});
};
...
router.post('/studentOnBoarding',(req,res)=>{
    ...
    getstudentWithScanId(obj).then(function(student){
      console.log("Student is registered in bus: ", student.busId, "and current busId is : ", obj.busId, {parentId : student.userId});
